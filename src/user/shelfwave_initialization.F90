...
subroutine shelfwave_initialize_topography( D, G, param_file, max_depth, US )
  ...
  ! Local variables
  real      :: y    ! Position relative to the southern boundary [km]
  real      :: rLy  ! Exponential decay rate of the topography [km-1]
  real      :: Ly   ! Exponential decay lengthscale of the topography [km]
  real      :: H0   ! The minimum depth of the ocean [Z ~> m]
  integer   :: i, j

  call get_param(param_file, mdl,"SHELFWAVE_Y_LENGTH_SCALE", Ly, &
                 units="km", default=50., scale=1.0, do_not_log=.true.)
