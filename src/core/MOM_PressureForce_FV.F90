        ...
        if (abs(pa(i+1,j,1) - pa(i,j,1)) > abs(0.25*(rho_top(I+1,j)+rho_top(I,j)-2.0*rho_ref) * &
          ...
          if (CS%correction_intxpa_5pt) then
            ...
            do m=2,4
              ...
              p5(m) = p5(m-1) + (0.25*(p5(5)-p5(1)) + 0.125*(r5(5)+r5(1))*GV%g_Earth*(e(I+1,j,1)-e(I,j,1)) - &
            ...
            correction_x(I,j) = -0.5*(pa(i,j,1) + pa(i+1,j,1)) + intx_pa(I,j,1)
      ...
      if (((pa(i,j+1,1)-pa(i,j,1))*(-e(i,j+1,1)+e(i,j,1))) < 0.0) then
        ...
        if (abs(pa(i,j+1,1) - pa(i,j,1)) > abs(0.25*(rho_top(i,J+1)+rho_top(i,J)-2.0*rho_ref) * &
          ...
          if (CS%correction_intxpa_5pt) then
            ...
            ! Get correction from difference between this and linear average. This is clunky and repetitive.
            correction_y(I,j) = -0.5*(pa(i,j,1) + pa(i,j+1,1)) + inty_pa(I,j,1)
          else ! Do not use 5-point quadrature.
