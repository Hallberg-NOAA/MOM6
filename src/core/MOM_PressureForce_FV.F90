...
subroutine PressureForce_FV_Bouss(h, tv, PFu, PFv, G, GV, US, CS, ALE_CSp, p_atm, pbce, eta)
  ...
  real, dimension(SZIB_(G),SZJ_(G)) :: &
    ...
    intx_pa_cor         ! The correction to intx_pa based on the reference interface calculations [R L2 T-2 ~> Pa]
  real, dimension(SZIB_(G),SZJ_(G)) :: &
    ...
    inty_pa_cor         ! The correction to inty_pa based on the reference interface calculations [R L2 T-2 ~> Pa]
  ...
  real :: G_x_Rho0           ! G_Earth * Rho0 in [R L2 Z-1 T-2 ~> Pa m-1].
  ...
  real :: pa5(5)       ! The pressure anomaly (i.e. pressure + g*RHO_0*e) at five
                       ! quadrature points [R L2 T-2 ~> Pa].
  ...
  if (use_p_atm) then
    !$OMP parallel do default(shared)
    do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
      pa(i,j,1) = G_x_Rho0*(e(i,j,1) - G%Z_ref) + p_atm(i,j)
    enddo ; enddo
  else
    !$OMP parallel do default(shared)
    do j=Jsq,Jeq+1 ; do i=Isq,Ieq+1
      pa(i,j,1) = G_x_Rho0*(e(i,j,1) - G%Z_ref)
    enddo ; enddo
  endif
      ...
      if (((pa(i+1,j,1)-pa(i,j,1))*(-e(i+1,j,1)+e(i,j,1))) < 0.0) then
        ...
        if (abs(pa(i+1,j,1) - pa(i,j,1)) > abs(0.25*(rho_top(I+1,j)+rho_top(I,j)-2.0*rho_ref) * &
          ...
          if (CS%correction_intxpa_5pt) then
            ...
            p5(1) = pa(i,j,1) - G_x_Rho0*(e(i,j,1) - G%Z_ref)
            p5(5) = pa(i+1,j,1) - G_x_Rho0*(e(i+1,j,1) - G%Z_ref)
            do m=2,4
              wt_R =  0.25*real(m-1)
            ...
            do m=2,4
              ...
              pa5(m) = pa5(m-1) + (0.25*(pa5(5)-pa5(1)) + 0.125*(r5(5)+r5(1))*GV%g_Earth*(e(I+1,j,1)-e(I,j,1)) - &
                                   (0.125*(r5(m)+r5(m-1))*GV%g_Earth*(e(I+1,j,1)-e(I,j,1))) )
            enddo
            ! Get a correction from difference between this and linear average.
            correction_x(I,j) = C1_90*((32.0*(pa5(2)+pa5(4)) + 12.0*pa5(3)) - 38.0*(pa5(1) + pa5(5)))
      ...
      if (((pa(i,j+1,1)-pa(i,j,1))*(-e(i,j+1,1)+e(i,j,1))) < 0.0) then
        ...
        if (abs(pa(i,j+1,1) - pa(i,j,1)) > abs(0.25*(rho_top(i,J+1)+rho_top(i,J)-2.0*rho_ref) * &
          ...
          if (CS%correction_intxpa_5pt) then
            ...
            ! Pressure input to density EOS is the actual pressure not adjusted for rho_ref.
            p5(1) = pa(i,j,1) - G_x_Rho0*(e(i,j,1) - G%Z_ref)
            p5(5) = pa(i,j+1,1) - G_x_Rho0*(e(i,j+1,1) - G%Z_ref)
            do m=2,4
              wt_R =  0.25*real(m-1)
              T5(m) = T5(1) + (T5(5)-T5(1))*wt_R !Quadratic: + (T5(5)-T5(1))*B*wt_R*(wt_R-1);
              S5(m) = S5(1) + (S5(5)-S5(1))*wt_R !+ (S5(5)-S5(1))*B*wt_R*(wt_R-1);
            ...
            do m=2,4
              ...
              ! Do this by integrating pressure between each of the 5 points and adding up
              ! This way integration direction doesn't matter when adding up pressure from previous point
              pa5(m) = pa5(m-1) + (0.25*(pa5(5)-pa5(1)) + 0.125*(r5(5)+r5(1))*GV%g_Earth*(e(i,j+1,1)-e(i,j,1)) - &
                                   (0.125*(r5(m)+r5(m-1))*GV%g_Earth*(e(i,j+1,1)-e(I,j,1))))
            enddo
            ! Get a correction from difference between this and linear average.
            correction_y(i,J) = C1_90*((32.0*(pa5(2)+pa5(4)) + 12.0*pa5(3)) - 38.0*(pa5(1) + pa5(5)))
          else ! Do not use 5-point quadrature.
  ...
  if (CS%reset_intxpa_integral) then
    ...
    intx_pa_nonlin(:,:) = 0.0 ; dgeo_x(:,:) = 0.0 ; intx_pa_cor(:,:) = 0.0
    do j=js,je ; do I=Isq,Ieq
    ...
    do k=1,nz-1
      ...
      do j=js,je ; do I=Isq,Ieq ; if (seek_x_cor(I,j)) then
        ...
        if (((h(i,j,k) > CS%h_nonvanished) .and. (h(i+1,j,k) > CS%h_nonvanished)) .and. &
            (max(0., e(i+1,j,K+1)-e(i,j,1), e(i,j,K+1)-e(i+1,j,1)) <= CS%dz_MWIPG)) then
            ...
            p_int_W(I,j) = pa(i,j,K+1) - G_x_Rho0*(e(i,j,K+1) - G%Z_ref)
            p_int_E(I,j) = pa(i+1,j,K+1) - G_x_Rho0*(e(i+1,j,K+1) - G%Z_ref)
    ...
    do j=js,je
      ...
      do I=Isq,Ieq
        ...
        intx_pa_cor(I,j) = C1_12 * (rho_x_E(I,j)-rho_x_W(I,j)) * dgeo_x(I,j) - intx_pa_nonlin(I,j)
      enddo
    enddo
    ...
    inty_pa_nonlin(:,:) = 0.0 ; dgeo_y(:,:) = 0.0 ; inty_pa_cor(:,:) = 0.0
    do J=Jsq,Jeq ; do i=is,ie
    ...
    do k=1,nz-1
      ...
      do J=Jsq,Jeq ; do i=is,ie ; if (seek_y_cor(i,J)) then
        ...
        if (((h(i,j,k) > CS%h_nonvanished) .and. (h(i,j+1,k) > CS%h_nonvanished)) .and. &
            (max(0., e(i,j+1,K+1)-e(i,j,1), e(i,j,K+1)-e(i,j+1,1)) <= CS%dz_MWIPG)) then
            ...
            p_int_S(i,J) = pa(i,j,K+1) - G_x_Rho0*(e(i,j,K+1) - G%Z_ref)
            p_int_N(i,J) = pa(i,j+1,K+1) - G_x_Rho0*(e(i,j+1,K+1) - G%Z_ref)
    ...
    do J=Jsq,Jeq
      ...
      do i=is,ie
        ...
        inty_pa_cor(i,J) = C1_12 * (rho_y_N(i,J)-rho_y_S(i,J)) * dgeo_y(i,J) - inty_pa_nonlin(i,J)
      enddo
    enddo

    ! Correct intx_pa and inty_pa at each interface using vertically constant corrections.
    do K=1,nz+1 ; do j=js,je ; do I=Isq,Ieq
      intx_pa(I,j,K) = intx_pa(I,j,K) + intx_pa_cor(I,j)
    enddo ; enddo ; enddo

    do K=1,nz+1 ; do J=Jsq,Jeq ; do i=is,ie
      inty_pa(i,J,K) = inty_pa(i,J,K) + inty_pa_cor(i,J)
    enddo ; enddo ; enddo
