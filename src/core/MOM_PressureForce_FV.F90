...
type, public :: PressureForce_FV_CS ; private
  ...
  real    :: h_MWIPG        !< A thickness between an interface and the potentially ice-depressed
                            !! sea surface height or pressure in a neighboring cell beyond which the mass-weighting
                            !! is enabled [H ~> m or kg m-2]
...
subroutine PressureForce_FV_nonBouss(h, tv, PFu, PFv, G, GV, US, CS, ALE_CSp, p_atm, pbce, eta)
  ...
  real :: dp_MWIPG      ! A change in pressure between an interface and the potentially ice-enhanced
                        ! sea surface pressure in a neighboring cell beyond which the mass-weighting
                        ! is enabled [R L2 T-2 ~> Pa]
...
!  real :: oneatm       ! 1 standard atmosphere of pressure in [R L2 T-2 ~> Pa]
  ...
  dp_MWIPG = H_to_RL2_T2 * CS%h_MWIPG
  ...
  if (CS%correction_intxpa) then
    ...
    if (CS%correction_intxpa_5pt) then
      ...
      ! This version makes a parabolic correction for hydrostatic variations in surface pressure under ice.

      ! Determine surface specific volume for use in the pressure gradient corrections
      do j=Jsq,Jeq+1
        call calculate_spec_vol(T_top(:,j), S_top(:,j), p(:,j,1), SpV_top(:,j), &
  ...
  if (CS%reset_intxpa_integral) then
    ...
    ! Note: This option currently assumes that pressure varies quadratically along the bottom of first
    ! nonvanished, nonMWIPG level.  Possibly 5 point quadrature should be implemented as for the surface.
    ...
    do k=1,nz-1
      ...
      do j=js,je ; do I=Isq,Ieq ; if (seek_x_cor(I,j)) then
        ! Find the topmost layer for which both sides are nonvanished and MWIPG is not activated
        if (((h(i,j,k) > CS%h_nonvanished) .and. (h(i+1,j,k) > CS%h_nonvanished)) .and. &
            (max(0., p(i,j,1)-p(i+1,j,K+1), p(i+1,j,1)-p(i,j,K+1)) <= dp_MWIPG)) then
    ...
    do j=js,je
      call calculate_spec_vol(T_int_W(:,j), S_int_W(:,j), p_int_W(:,j), SpV_x_W(:,j), &
                              tv%eqn_of_state, EOSdom_u, spv_ref=alpha_ref)
      call calculate_spec_vol(T_int_E(:,j), S_int_E(:,j), p_int_E(:,j), SpV_x_E(:,j), &
                              tv%eqn_of_state, EOSdom_u, spv_ref=alpha_ref)
      do I=Isq,Ieq
        ! This expression assumes that specific volume varies linearly with depth between the corners of the
        ! reference interfaces found above to get a vertically uniform correction to intx_za.
        ! This can be used without masking because dp_int_x and intx_za_nonlin are 0 over land.
        intx_za_cor_ri(I,j) = C1_12 * (SpV_x_E(I,j)-SpV_x_W(I,j)) * dp_int_x(I,j) - intx_za_nonlin(I,j)
      enddo
    enddo
    ...
    do k=1,nz-1
      ...
      do J=Jsq,Jeq ; do i=is,ie ; if (seek_y_cor(i,J)) then
        ...
        if (((h(i,j,k) > CS%h_nonvanished) .and. (h(i,j+1,k) > CS%h_nonvanished)) .and. &
            (max(0., p(i,j,1)-p(i,j+1,K+1), p(i,j+1,1)-p(i,j,K+1)) <= dp_MWIPG)) then
    ...
    do J=Jsq,Jeq
      call calculate_spec_vol(T_int_S(:,J), S_int_S(:,J), p_int_S(:,J), SpV_y_S(:,J), &
                             tv%eqn_of_state, EOSdom_v)
      call calculate_spec_vol(T_int_N(:,J), S_int_N(:,J), p_int_N(:,J), SpV_y_N(:,J), &
                             tv%eqn_of_state, EOSdom_v)
      do i=is,ie
        ! This expression assumes that specific volume varies linearly with depth between the corners of the
        ! reference interfaces found above to get a vertically uniform correction to inty_pa.
        ! This can be used without masking because dgeo_y and inty_pa_nonlin are 0 over land.
        inty_za_cor_ri(i,J) = C1_12 * (SpV_y_N(i,J)-SpV_y_S(i,J)) * dp_int_y(i,J) - inty_za_nonlin(i,J)
      enddo
    enddo
...
subroutine PressureForce_FV_Bouss(h, tv, PFu, PFv, G, GV, US, CS, ALE_CSp, p_atm, pbce, eta)
  ...
  real :: dz_MWIPG           ! A vertical distance between an interface and the potentially ice-depressed
                             ! sea surface height in a neighboring cell beyond which the mass-weighting
                             ! is enabled [Z ~> m]
  real :: H_to_RL2_T2        ! A factor to convert from thickness units (H) to pressure
                             ! units [R L2 T-2 H-1 ~> Pa m-1 or Pa m2 kg-1].
  logical :: use_p_atm       ! If true, use the atmospheric pressure.
  ...
  if (CS%reset_intxpa_integral) then
    ...
    do k=1,nz-1
      ...
      do j=js,je ; do I=Isq,Ieq ; if (seek_x_cor(I,j)) then
        ! Find the topmost layer for which both sides are nonvanished and MWIPG is not activated
        if (((h(i,j,k) > CS%h_nonvanished) .and. (h(i+1,j,k) > CS%h_nonvanished)) .and. &
            (max(0., e(i+1,j,K+1)-e(i,j,1), e(i,j,K+1)-e(i+1,j,1)) <= dz_MWIPG)) then
            ! Store properties at the bottom of this cell to get a "good estimate" for intxpa at
            ! the interface below this cell (it might have quadratic pressure dependence if sloped)
            T_int_W(I,j) = T_b(i,j,k) ; T_int_E(I,j) = T_b(i+1,j,k)
            S_int_W(I,j) = S_b(i,j,k) ; S_int_E(I,j) = S_b(i+1,j,k)
    ...
    do j=js,je
      call calculate_density(T_int_W(:,j), S_int_W(:,j), p_int_W(:,j), rho_x_W(:,j), &
                             tv%eqn_of_state, EOSdom_u)
      call calculate_density(T_int_E(:,j), S_int_E(:,j), p_int_E(:,j), rho_x_E(:,j), &
                             tv%eqn_of_state, EOSdom_u)
      do I=Isq,Ieq
        ! This expression assumes that density varies linearly with depth between the corners of the
        ! reference interfaces found above to get a vertically uniform correction to intx_pa.
        ! This can be used without masking because dgeo_x and intx_pa_nonlin are 0 over land.
        intx_pa_cor_ri(I,j) = C1_12 * (rho_x_E(I,j)-rho_x_W(I,j)) * dgeo_x(I,j) - intx_pa_nonlin(I,j)
      enddo
    enddo
    ...
    do k=1,nz-1
      ...
      do J=Jsq,Jeq ; do i=is,ie ; if (seek_y_cor(i,J)) then
        ! Find the topmost layer for which both sides are nonvanished and MWIPG is not activated
        if (((h(i,j,k) > CS%h_nonvanished) .and. (h(i,j+1,k) > CS%h_nonvanished)) .and. &
            (max(0., e(i,j+1,K+1)-e(i,j,1), e(i,j,K+1)-e(i,j+1,1)) <= dz_MWIPG)) then
            ! Store properties at the bottom of this cell to get a "good estimate" for intypa at
            ! the interface below this cell (it might have quadratic pressure dependence if sloped)
            T_int_S(i,J) = T_b(i,j,k) ; T_int_N(i,J) = T_b(i,j+1,k)
            S_int_S(i,J) = S_b(i,j,k) ; S_int_N(i,J) = S_b(i,j+1,k)
    ...
    do J=Jsq,Jeq
      call calculate_density(T_int_S(:,J), S_int_S(:,J), p_int_S(:,J), rho_y_S(:,J), &
                             tv%eqn_of_state, EOSdom_v)
      call calculate_density(T_int_N(:,J), S_int_N(:,J), p_int_N(:,J), rho_y_N(:,J), &
                             tv%eqn_of_state, EOSdom_v)
      do i=is,ie
        ! This expression assumes that density varies linearly with depth between the corners of the
        ! reference interfaces found above to get a vertically uniform correction to inty_pa.
        ! This can be used without masking because dgeo_y and inty_pa_nonlin are 0 over land.
        inty_pa_cor_ri(i,J) = C1_12 * (rho_y_N(i,J)-rho_y_S(i,J)) * dgeo_y(i,J) - inty_pa_nonlin(i,J)
      enddo
    enddo
...
# include "version_variable.h"
  ...
  call get_param(param_file, mdl, "CORRECTION_INTXPA_5PT", CS%correction_intxpa_5pt, &
                 ...
                 "CORRECTION_INTXPA = True.", default=.false., do_not_log=.not.use_EOS)
  ...
  if (.not.use_EOS) then  ! These options do nothing without an equation of state.
    ...
    CS%correction_intxpa_5pt = .false.
  ...
  call get_param(param_file, mdl, "RESET_INTXPA_MASS_WEIGHT_CLEARANCE", CS%h_MWIPG, &
                 ...
                 default=1.0e-8, units="m", scale=GV%m_to_H, do_not_log=.not.CS%reset_intxpa_integral)
