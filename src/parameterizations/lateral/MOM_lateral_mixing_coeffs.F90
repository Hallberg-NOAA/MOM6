...
subroutine calc_QG_Leith_viscosity(CS, G, GV, US, h, k, div_xx_dx, div_xx_dy, slope_x, slope_y, &
  ...
  if ((k > 1) .and. (k < nz)) then
    ...
    do j=js-2,je+2 ; do I=is-2,ie+1
      ...
      dslopex_dz(I,j) = 2. * ( slope_x(I,j,k) - slope_x(I,j,k+1) ) * Ih
    ...
    do J=js-2,je+1 ; do i=is-2,ie+2
      ...
      Ih = 1./ ( ( h_at_slope_above + h_at_slope_below + GV%H_subroundoff ) * GV%H_to_Z )
      dslopey_dz(i,J) = 2. * ( slope_y(i,J,k) - slope_y(i,J,k+1) ) * Ih
